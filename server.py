...
def load_devices():
    store = load_json(DEVICES_FILE, {"next_id": 1, "devices": {}})
...
def mikrotik_push():
    ...
    (ไม่มี auth เพราะ MikroTik ล็อกอินไม่ได้ — เหมือนระบบต้นแบบ)
...
@login_required
def api_server_info():
    """URL ที่ MikroTik ต้องยิงเข้ามา — ต้องเป็น IP ของเครื่องเซิร์ฟเวอร์ในวง LAN
    ไม่ใช่ 127.0.0.1/localhost ที่เห็นในแถบ address ของเบราว์เซอร์"""
